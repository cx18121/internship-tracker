...
const db = new Database(path.join(process.cwd(), 'data', 'internships.db'));
const rows = db.prepare<[], { id: string; source: string; description: string | null }>(
...
// Show before/after HEAD for top reductions — preamble-skip cases will show
// the marketing prefix disappear from the start.
const sorted = rows
