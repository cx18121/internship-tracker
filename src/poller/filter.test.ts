...
describe('Filter', () => {
  test('Non-US location (London, UK) → excluded', () => {
    const r = applyHardFilters(posting({ title: 'SWE Intern', location: 'London, UK' }));
    assert.equal(r.passed, false);
    assert.equal(r.reason, 'non-us');
  });

  test('US location (New York, NY) → passes', () => {
    const r = applyHardFilters(posting({ title: 'SWE Intern', location: 'New York, NY' }));
  ...
  test('Closed posting (🔒) → excluded', () => {
    const r = applyHardFilters(posting({ title: '🔒 Backend Engineer Intern', location: 'Remote' }));
    assert.equal(r.passed, false);
    assert.equal(r.reason, 'closed');
  });

  test('Non-SWE role (Marketing Intern) → excluded', () => {
    const r = applyHardFilters(posting({ title: 'Marketing Intern', location: 'San Francisco, CA' }));
    assert.equal(r.passed, false);
    assert.equal(r.reason, 'non-swe');
  });

  test('SWE role (Backend Engineer Intern) → passes', () => {
    const r = applyHardFilters(posting({ title: 'Backend Engineer Intern', location: 'San Francisco, CA' }));
    assert.equal(r.passed, true);
  });
  ...
  test('Non-US: "Cambridge, United Kingdom" → excluded', () => {
    const r = applyHardFilters(posting({ location: 'Cambridge, United Kingdom' }));
    assert.equal(r.reason, 'non-us');
  });

  test('Non-US: "Hsinchu, Taiwan" → excluded', () => {
    const r = applyHardFilters(posting({ location: 'Hsinchu, Taiwan' }));
    assert.equal(r.reason, 'non-us');
  });

  test('Non-US: "Moscow, Russia" → excluded (common name, not "Russian Federation")', () => {
    const r = applyHardFilters(posting({ location: 'Moscow, Russia' }));
    assert.equal(r.reason, 'non-us');
  });

  test('Non-US: "Abidjan, Ivory Coast" → excluded (common, not "Côte d\'Ivoire")', () => {
    const r = applyHardFilters(posting({ location: 'Abidjan, Ivory Coast' }));
    assert.equal(r.reason, 'non-us');
  });

  test('Non-US: "Edinburgh, Scotland" → excluded (UK sub-national)', () => {
    const r = applyHardFilters(posting({ location: 'Edinburgh, Scotland' }));
    assert.equal(r.reason, 'non-us');
  });

  test('US: "Las Cruces, New Mexico" → passes (state name beats "mexico" substring)', () => {
    const r = applyHardFilters(posting({ location: 'Las Cruces, New Mexico' }));
    assert.equal(r.passed, true);
  });
  ...
  test('Non-US: "DE - Berlin" → excluded (country-first with foreign city)', () => {
    const r = applyHardFilters(posting({ location: 'DE - Berlin' }));
    assert.equal(r.reason, 'non-us');
  });

  test('Non-US: "CA-ON-MISSISSAUGA-..." → excluded (hierarchical code chain)', () => {
    const r = applyHardFilters(posting({ location: 'CA-ON-MISSISSAUGA-P22M01' }));
    assert.equal(r.reason, 'non-us');
  });

  test('Non-US: "IN-Pune" → excluded (foreign city beats Indiana state code)', () => {
    const r = applyHardFilters(posting({ location: 'IN-Pune' }));
    assert.equal(r.reason, 'non-us');
  });

  test('US: "AZ - Chandler" → passes (state-prefix form, no foreign city)', () => {
    const r = applyHardFilters(posting({ location: 'AZ - Chandler' }));
    assert.equal(r.passed, true);
  });

  test('US: "CA - San Francisco" → passes (California prefix, US city)', () => {
    const r = applyHardFilters(posting({ location: 'CA - San Francisco' }));
    assert.equal(r.passed, true);
  });
});
...
describe('Season expiry (applyHardFilters)', () => {
  test('applyHardFilters uses the explicit season token for expiry, not just the title', () => {
    const expired = applyHardFilters(posting({ title: 'Software Engineer Intern', location: 'NYC', season: ['winter-2024'] }));
    ...
    const live = applyHardFilters(posting({ title: 'Software Engineer Intern', location: 'NYC', season: ['fall-2027'] }));
  ...
  test('applyHardFilters: rejects expired-season SWE roles, keeps far-future ones', () => {
    ...
    const expired = applyHardFilters(posting({ title: 'Software Engineer Intern - Summer 2020', location: 'Remote in USA' }));
    assert.equal(expired.passed, false);
    assert.equal(expired.reason, 'expired-season');
    const future = applyHardFilters(posting({ title: 'Software Engineer Intern - Summer 2099', location: 'Remote in USA' }));
